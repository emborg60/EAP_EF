...
def get_error_metrics(df,
    ...
    df = add_lags(df, N, ['positive_comment'])
    df = add_lags(df, N, ['neutral_comment'])
    df = add_lags(df, N, ['negative_comment'])
    ...
    for n in range(N, 0, -1):
        ...
        features.append("positive_comment_lag_" + str(n))
        features.append("neutral_comment_lag_" + str(n))
        features.append("negative_comment_lag_" + str(n))
...
# fig = dict(data=data, layout=layout)
# py.plot(fig, filename='price_grouped_month')

### No clear structure from month. (slightly over the summer.)

# # Compute the average price for each day of month
# avg_price_day = df.groupby("day").agg({'price': 'mean'}).reset_index()
#
# # Plot
# data = [go.Scatter(
#             x = avg_price_day['day'],
#             y = avg_price_day['price'],
#             mode = 'markers',
#             marker=dict(
#                 color='LightSkyBlue',
#                 size=15,
...
#
# fig = dict(data=data, layout=layout)
# py.plot(fig, filename='price_grouped_dayofmonth')

## Again no clear structure.

#
# # Compute the average price for each day of week
# avg_price_dayofweek = df.groupby("dayofweek").agg({'price': 'mean'}).reset_index()
#
# # Plot
# data = [go.Scatter(
#             x = avg_price_dayofweek['dayofweek'],
#             y = avg_price_dayofweek['price'],
#             mode = 'markers',
#             marker=dict(
#                 color='LightSkyBlue',
#                 size=15,
...
#
# fig = dict(data=data, layout=layout)
# py.plot(fig, filename='price_grouped_dayofweek')
#
# fig = px.box(df, x="dayofweek", y="price")
# py.plot(fig, filename='price_boxplot_dayofweek')
#
# fig = px.box(df, x="day", y="price")
# py.plot(fig, filename='price_boxplot_dayofmonth')
#
# fig = px.box(df, x="month", y="price")
# py.plot(fig, filename='price_boxplot_month')

train = df[pred_day-train_val_size:pred_day-val_size].copy()
df_lags = add_lags(train, N, ['price'])

 df_lags = add_lags(df_lags, 3, ['volatility'])
 df_lags = add_lags(df_lags, 3, ['volume_price'])
 df_lags = add_lags(df_lags, 3, ['volume_number'])
 df_lags = add_lags(df_lags, 3, ['positive_comment'])
 df_lags = add_lags(df_lags, 3, ['neutral_comment'])
 df_lags = add_lags(df_lags, 3, ['negative_comment'])
#
#
...
# Plot correlation for price lag features only
features = ['price']
...
fig = go.Figure(data=go.Heatmap(
    ...
    zmax = 1,
    zmin = -1))
py.plot(fig, filename='price_lag_corr')
...
#     x=features,
#     y=features))
# py.plot(fig, filename='price_date_corr')
#
# plot correlation for sentiment features:
features = ['price']
for n in range(1, N + 1, 1):
    features.append("positive_comment_lag_" + str(n))
for n in range(1, N + 1, 1):
...
fig = go.Figure(data=go.Heatmap(
    ...
    zmax=1,
    zmin=-1))
py.plot(fig, filename='price_senti_corr')
...
test = df[pred_day:]
train = df[pred_day-train_val_size:pred_day-val_size].copy()
fig.add_trace(go.Scatter(x=train_val['date'],
                         y=train_val['price'],
                         mode='lines',
                         name='train',
...
py.plot(fig, filename='price_train_test')
...
py.plot(fig, filename='price_val_untuned')
...
### TODO - tilføj normdist for at tjekke robustness af model. ####


################ Tuning n_estimators, max_depht
param_label = 'n_estimators'
param_list = [200, 175, 150, 125, 100, 75, 50, 25]
...
fig.update_xaxes(range=['2017-10-16', '2018-11-12'])
fig.update_yaxes(range=[127, 157])
py.plot(fig, filename='price_val_' + str(sfromdate) + '_' + str(stodate))
...
fig.update_layout(yaxis=dict(title='USD'),
                  xaxis=dict(title='date'))
py.plot(fig, filename='price_fc_' + str(sfromdate) + '_' + str(stodate))
...
fig.update_layout(yaxis=dict(title='feature'),
                   xaxis=dict(title='relative importance'))
py.plot(fig, filename='imp_' + str(sfromdate) + '_' + str(stodate))
...
# pickle.dump(rmse_bef_tuning, open("./out/senti/measures/val_rmse_bef_tuning_" + df.iloc[pred_day]['date'].strftime("%Y-%m-%d") + '-' + df.iloc[pred_day+H]['date'].strftime("%Y-%m-%d") + ".pickle", "wb"))
# pickle.dump(rmse_aft_tuning, open("./out/senti/measures/val_rmse_aft_tuning_" + df.iloc[pred_day]['date'].strftime("%Y-%m-%d") + '-' + df.iloc[pred_day+H]['date'].strftime("%Y-%m-%d")  +".pickle", "wb"))
# pickle.dump(test_rmse_bef_tuning, open("./out/senti/measures/test_rmse_bef_tuning_" + df.iloc[pred_day]['date'].strftime("%Y-%m-%d") + '-' + df.iloc[pred_day+H]['date'].strftime("%Y-%m-%d") + ".pickle", "wb"))
# pickle.dump(test_mape_bef_tuning, open("./out/senti/measures/test_mape_bef_tuning_" + df.iloc[pred_day]['date'].strftime("%Y-%m-%d") + '-' + df.iloc[pred_day+H]['date'].strftime("%Y-%m-%d") + ".pickle", "wb"))
# pickle.dump(test_mae_bef_tuning, open("./out/senti/measures/test_mae_bef_tuning_" + df.iloc[pred_day]['date'].strftime("%Y-%m-%d") + '-' + df.iloc[pred_day+H]['date'].strftime("%Y-%m-%d") +  ".pickle", "wb"))
# pickle.dump(test_rmse_aft_tuning, open("./out/senti/measures/test_rmse_aft_tuning_" + df.iloc[pred_day]['date'].strftime("%Y-%m-%d") + '-' + df.iloc[pred_day+H]['date'].strftime("%Y-%m-%d") + ".pickle", "wb"))
# pickle.dump(test_mape_aft_tuning, open("./out/senti/measures/test_mape_aft_tuning_" + df.iloc[pred_day]['date'].strftime("%Y-%m-%d") + '-' + df.iloc[pred_day+H]['date'].strftime("%Y-%m-%d") + ".pickle", "wb"))
# pickle.dump(test_mae_aft_tuning, open("./out/senti/measures/test_mae_aft_tuning_" + df.iloc[pred_day]['date'].strftime("%Y-%m-%d") + '-' + df.iloc[pred_day+H]['date'].strftime("%Y-%m-%d") + ".pickle", "wb"))
# pickle.dump(est, open("./out/senti/forecasts/test_est_aft_tuning_" + df.iloc[pred_day]['date'].strftime("%Y-%m-%d") + '-' + df.iloc[pred_day+H]['date'].strftime("%Y-%m-%d") +  ".pickle", "wb"))
#
