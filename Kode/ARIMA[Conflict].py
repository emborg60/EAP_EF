...
while daily_re.index[pred_day] < daily_re.index[len(daily_re) - 1]:
    ...
    chosen_paramters.append(best_param)
...
# Export predictions
da_fore.to_csv(r'Data\ARIMA_Pred.csv')
...
price = daily_re.iloc[:, 0]
da_price = daily_re.iloc[:, 0]
daily_re.drop(daily_re.columns[[0, 5, 6, 7]], axis=1, inplace=True)
y = daily_re.iloc[:, -1]
print(daily_re.head())
...
pred.index.value_counts().max()

# Export predictions
da_fore.to_csv(r'Data\ARIMA_Pred_NoSent.csv')
...
while daily_re.index[pred_day] < daily_re.index[len(daily_re) - 1]:
    ...
    model = sm.tsa.ARIMA(y_da_train, order=(1, 1, 0)).fit(disp=-1, trend = 'nc')
    ar_coef = model.arparams
    history = [x for x in y_da_train]
    predictions = list()
    for t in range(len(y_da_test)):
        yhat = predict(ar_coef, history)
        predictions.append(yhat)
        obs = y_da_test[t]
        history.append(obs)

    append = pd.DataFrame(predictions)
    pred = pd.concat([pred, append], ignore_index=True)
...
new_bm = pred[['pred']]
new_bm = new_bm.merge(df[['price']], how = 'left', left_index=True, right_index=True)
new_bm = new_bm[:-16]
sqrt(mean_squared_error(new_bm['price'], new_bm['pred']))
...
# ['volatility']
series_analysis(df, 'volatility', xlabel='year', ylabel='volatility (daily (high-low)/price)',
                title='df[\'volatility\']')
# ADF: Stationary
# KPSS: Not Stationary
...
# ['volume_number']
series_analysis(df, 'volume_number', xlabel='year', ylabel='Number of Bitocins exchanged',
                title='df[\'volume_number\']')
# ADF: "Not Stationary"
# KPSS: "Not Stationary"
...
# ['positive_comment']
series_analysis(df, 'positive_comment', xlabel='year', ylabel='Number of Positive Comments',
                title='df[\'positive_comment\']')
# ADF: "Stationary"
# KPSS: "Not Stationary"
...
# run tests to see if stationary
series_analysis(stationary, 'positive_comment', xlabel='year', ylabel='Number of Positive Comments',
                title='Diff_Positive_Comment')
# ADF: "Stationary"
# KPSS: "Stationary"


# ['neutral_comment']
series_analysis(df, 'neutral_comment', xlabel='year', ylabel='Number of Neutral Comments',
                title='df[\'neutral_comment\']')
# ADF: "Stationary"
# KPSS: "Not Stationary"
...
# run tests to see if stationary
series_analysis(stationary, 'neutral_comment', xlabel='year', ylabel='Number of Neutral Comments',
                title='Diff_Neutral_Comment')
# ADF: "Stationary"
# KPSS: "Stationary"


# ['negative_comment']
series_analysis(df, 'negative_comment', xlabel='year', ylabel='Number of Negative Comments',
                title='df[\'negative_comment\']')
# ADF: "Stationary"
# KPSS: "Not Stationary"
...
# run tests to see if stationary
series_analysis(stationary, 'negative_comment', xlabel='year', ylabel='Number of Negative Comments',
                title='Diff_Negative_Comment')
# ADF: "Stationary"
# KPSS: "Stationary"
